...
export function renderRoutineEditorView(container) {
  ...
  container.innerHTML = `
        <div data-view="routine-editor" class="space-y-6 active">
            <div class="flex items-center gap-4">
                <button class="back-to-routines-btn text-2xl">&larr;</button>
                <h2 class="text-3xl font-bold">${
                  state.editingItem ? "Routine bearbeiten" : "Neue Routine"
                }</h2>
            </div>
            <form id="routine-form" class="space-y-4 p-4 rounded-xl" style="background-color: var(--bg-secondary);">
                <input name="name" placeholder="Name der Routine" value="${
                  state.editingItem?.name || ""
                }" class="w-full p-3 border rounded-lg" style="background-color: var(--bg-primary); border-color: var(--border-color);" required/>
...
function renderRoutineItems() {
  ...
  if (!itemsList) return;
  itemsList.innerHTML = routineItems
    .map(
      (item, index) => `
...
function setupRoutineEditorListeners() {
  document.querySelector(".back-to-routines-btn").onclick = () =>
    switchView("settings-routines");
  document.getElementById("add-routine-item-btn").onclick = () =>
    openFormModal("routine-item");
